...
All of the data used in this project is included in the repository in CSV form, including the fast food only restaurant inspection dataset, the NYC farmers markets dataset, and the statewide retail food stores dataset. I only uploaded the cleaned fast-food dataset that I created from the restaurant inspection data, the original was too large. All processing and visualizations were done in my R notebook titled PDP.ipynb, which contains the code used to clean the data, filter it to NYC ZIP codes, identify full-service grocery stores, and create the maps, bar charts, and summary statistics used in the project.
